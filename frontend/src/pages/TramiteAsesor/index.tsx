...
import { getTramites, login,deleteRol,createRol} from "@/pages/services/tramiteasesor.services";
...
const FilteredUnidad: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (authData) {
        const tokenResponse = authData.token;
        setToken(tokenResponse);
        
        // Obtener tramites
        const response = await getTramites(tokenResponse);
        const tramitesArray = Object.values(response.tramite_actividades).flat();
        setData(tramitesArray); // Establece data como un array aplanado
...
//  };  
        ...
        <table className="w-full table-auto border-collapse">
          <thead className="bg-gray-50">
            <tr className="bg-primary text-left text-white">
              {['Tramite','Actividad','Estudiante','Titulo'].map((key) => (
                <th 
                  key={key}
  ...
  {filteredData.map((item) => (
    <tr key={`${item.idTramite}-${item.idActividad}`} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200">
      <td className="border-b border-gray-200 py-4 px-6">{item.idTramite}</td>
      <td className="border-b border-gray-200 py-4 px-6">{item.idActividad}</td>
      <td className="border-b border-gray-200 py-4 px-6">Alumno</td>
      <td className="border-b border-gray-200 py-4 px-6">Titulo</td>
