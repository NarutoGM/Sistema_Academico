...
import { getRoles, login,deleteRol,createRol} from "@/pages/services/tramite.services";
...
const FilteredUnidad: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const tramites = await getRoles(tokenResponse);
      setData(tramites);
        ...
        <table className="w-full table-auto border-collapse">
          <thead className="bg-gray-50">
            <tr className="bg-primary text-left text-white">
              {['Flujo','Unidad', 'Especialidad','Asesor'].map((key) => (
                <th 
                  key={key}
          ...
          <tbody>
            {filteredData.map((item) => (
              ...
              //'Fecha de Presentación', 'Fecha de Entrega Programada','Fecha de Entrega Real','Persona',
             // 'Unidad','Flujo','Observaciones','Producto','Especialidad'
              <tr key={item.id} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200">
                                <td className="border-b border-gray-200 py-4 px-6">{item.idFlujo}</td>

                                <td className="border-b border-gray-200 py-4 px-6">{item.idUnidad}</td>
                <td className="border-b border-gray-200 py-4 px-6">{item.idEspecialidad}</td>
                <td className="border-b border-gray-200 py-4 px-6">{item.idPersona}</td>
