...
const ReporteDocentesFilial: React.FC<ReporteDocentesFilialProps> = ({ docentesFilial }) => {
    ...
    if (chartRef.current) {
      ...
      if (ctx) {
        ...
        chartInstanceRef.current = new Chart(ctx, {
          type: 'bar',
          data: {
            labels: ['Solo Filial Trujillo', 'Solo Filial Valle Jequetepeque', 'Ambas Filiales'],
            datasets: [
              {
                label: 'Número de Docentes',
                data: [docentesSoloFilial1, docentesSoloFilial3, docentesAmbasFiliales],
  ...
  const generarPDF = () => {
    ...
    setTimeout(() => {
      ...
      doc.setFontSize(16);
      doc.text('Reporte de Docentes por Ambas Filiales', 105, 10, { align: 'center' });
      ...
      doc.autoTable({
        startY: 25,
        head: [['ID', 'Nombre', 'Filial Trujillo', 'Filial Valle Jequetepeque', 'Ambas Filiales']],
